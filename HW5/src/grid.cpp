...
bool Grid::intersect(const Ray& r, Hit& hit, float tmin) {
    ...
    //ray trace
    if (mi.cur_t < hit.getT()) {
        while (mi.grid_i >= 0 && mi.grid_j >= 0 && mi.grid_k >= 0 && mi.grid_i < nx && mi.grid_j < ny && mi.grid_k < nz) {
            int i = mi.grid_i;
            int j = mi.grid_j;
            int k = mi.grid_k;
            ...
            if (!opaque[index].empty()) {
                ...
                hit.set(mi.cur_t, m, mi.normal, r);
                return true;
...
void Grid::initializeRayMarch(MarchingInfo& mi, const Ray& r, float tmin) const {
    ...
    //mi.dt_x
    mi.d_tx = fabs(cellx / rd.x());
    mi.d_ty = fabs(celly / rd.y());
    mi.d_tz = fabs(cellz / rd.z());
    ...
    mi.grid_i = int(p.x() / cellx);
    if (mi.sign_x < 0 && mi.grid_i == nx) mi.grid_i--;
    mi.grid_j = int(p.y() / celly);
    if (mi.sign_y < 0 && mi.grid_j == ny) mi.grid_j--;
    mi.grid_k = int(p.z() / cellz);
    if (mi.sign_z < 0 && mi.grid_k == nz) mi.grid_k--;
